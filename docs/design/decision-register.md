...
| MDR-062 | Payroll UI is history-preservingly separated into `srinivasbs2000/hrms-payroll-web`; `hrms-payroll` remains the backend/program/API/OpenAPI/database/Keycloak governance authority, the web repository owns frontend CI/Dependabot/SBOM/browser E2E, and no third contract repository is introduced | Architecture/Delivery | IMPLEMENTED | HK-UI-SPLIT-01A seam PR #41; UI extraction/provenance; UI PR #1 / `ac677b4de57f9620a3ad255e5d72a406dc8f6c53`; HK-UI-SPLIT-01D source cleanup/closure | Preserve two-repository ownership and cross-repository E2E; current migration authority is MDR-064 |
...
| MDR-064 | P5-FSR-01 — Foundation Snapshot & Readiness Closure is the active bounded capability after activation-authority merge; V036 is reserved exclusively to it and no other capability may allocate V036 while active | Product/Governance | APPROVED | R3 reconciliation 10 Aug 2026; owner activation request; P5-FSR-01 scope authority | Create product branch only from activation-merged main; release V036 only at status closure |
