...
public record PayComponentCreateRequest(
    @NotBlank @Pattern(regexp = "^[A-Z][A-Z0-9_]{1,39}$") String code,
    @NotBlank @Size(max = 160) String name,
    @NotBlank @Pattern(regexp = "^(EARNING|DEDUCTION|INFORMATION)$")
        String componentType,
    String ownershipScope,
    @Pattern(regexp = "^[A-Z]{2}$") String countryCode,
    Boolean protectedFlag,
    String confidentialityLevel,
    @NotNull @Valid PayComponentVersionWriteRequest version) {
  ...
  public void validate() {
    ...
    if (!Set.of("EARNING", "DEDUCTION", "INFORMATION").contains(componentType)) {
