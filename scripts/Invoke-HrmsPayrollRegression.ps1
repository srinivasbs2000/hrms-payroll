...
if (-not $SkipFrontend) {
    if (
        ...
        throw (
            'Frontend repository not found: {0}' -f
    ...
    try {
        Write-Step 'Frontend dependency installation'
        & npm.cmd ci
        Assert-ExitCode 'Frontend npm ci'

        Write-Step 'Frontend audit-policy self-test'
        & node.exe scripts/verify-npm-audit.mjs --self-test
        Assert-ExitCode 'Frontend audit-policy self-test'

        Write-Step 'Frontend audit policy'
        & node.exe scripts/verify-npm-audit.mjs
        Assert-ExitCode 'Frontend audit policy'
...
Write-Host ''
Write-Host 'Backend working-tree status:'
git -C $RepoRoot status --short
Assert-ExitCode 'backend git status'

if (-not $SkipFrontend) {
    Write-Host ''
    Write-Host 'Frontend working-tree status:'
    git -C $FrontendRoot status --short
    Assert-ExitCode 'frontend git status'
}

Write-Host ''
Write-Host 'HRMS Payroll cross-repository regression passed.' -ForegroundColor Green
